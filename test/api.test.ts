...
import { appOf, MockMcp } from "./helpers.js";
...
describe("errors", () => {
  it("OC missing is not HTTP 200 empty results", async () => {
    const mcp = new MockMcp();
    const { app, sid } = await appOf(mcp, false);
    const res = await app.inject({
      method: "POST",
      url: "/v1/research",
      payload: { query: "민법" },
  ...
  it("MCP down => MCP_UNAVAILABLE", async () => {
    ...
    const { app, sid } = await appOf(mcp, true);
    const res = await app.inject({
      method: "POST",
      url: "/v1/research",
      payload: { query: "민법" },
